...
24 суда первой инстанции и областной суд: поиск закрыт проверочным кодом,
новые дела добавляют операторы; карточки остаются включены в мониторинг.
Седьмой КСОЮ ищется автоматически с фильтром по судам этой территории.
Президиум — дополнительный кассационный раздел областного суда, также
с операторским поиском. ХМАО, ЯНАО и отдельные мировые участки не подключены.
...
# Поиск с капчей выполняет оператор; импортированные карточки перечитываются
# штатным мониторингом кассации по домену и разделу 2800001.
