...
переопределяет `CM_PARALLEL_STAGGER_SECONDS=300` через systemd drop-in
`50-stagger-trial.conf` (проверено 20.09.2026). При списке
Урал → ХМАО → Башкортостан → Тюмень первые старты — 06:00/06:05/06:10/06:15
