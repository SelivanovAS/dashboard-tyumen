...
Код поддерживает ХМАО, Урал, Башкортостан и Тюменскую область.
Подключение четвёртого VPS-клона и фактическое расписание проверены 20.09.2026
в [runbook Тюмени](../../ops/vps-run/tyumen-bootstrap.md).
Подключение третьего VPS-клона и
...
списка территорий. По умолчанию интервал — 600 секунд, но действующий VPS
переопределяет `CM_PARALLEL_STAGGER_SECONDS=300`. С 20.09.2026 systemd drop-in
`60-region-starts.conf` также задаёт `CM_PARALLEL_START_DELAYS`:
Урал — 0, ХМАО — 300, Башкортостан и Тюмень — по 600 секунд.
Первые старты — 06:00/06:05/06:10/06:10
по Екатеринбургу (UTC+5). Каждый клон имеет свои Git, данные, журнал
