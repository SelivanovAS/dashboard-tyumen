...
Форма передаёт выбранные `section` и `delo_id` вместе с доменом суда.
Worker и импортёр сверяют раздел со ссылками карточек дампа: чужой раздел
или смесь апелляции и президиума отклоняются до импорта. Старые задания без
этих полей сохраняют определение раздела по ссылкам карточек.
Админка различает обычную кассацию КСОЮ и президиум:
...
- `needs_review` — неоднозначный суд, площадка, УИД или связь с первой инстанцией: интерфейс направляет
...
Башкортостане КСОЮ закреплён первым.

С 20.09.2026 успешный дамп обновляет только свой раздел:
`import:last:<домен>:<delo_id>`. `/admin/import-log` возвращает эти отметки
в `last_sections`, а прежние — в `last`. Апелляция (`5`) и президиум
(`2800001`) имеют независимые даты, счётчики и ошибки чтения карточек.
Старую отметку без раздела интерфейс использует только для однозначного суда;
у общего домена она не делает свежими сразу две инстанции. Журнал с
достоверным `section` позволяет восстановить отметку соответствующего раздела.
Площадки одного раздела сохраняют общий регламент свежести.

С 21.09.2026 результат с `needs_review > 0` не обновляет свежесть ни в Worker,
ни при восстановлении из журнала или браузерного кэша. Счётчик передаётся
также из точечного добавления. Нулевое число добавленных дел само по себе
не означает «всё уже в базе»: интерфейс показывает причины из отчёта.

Тюменский областной суд также имеет два источника: апелляцию и президиум.
Оба доступны через дамп; новые задания проверяются VPS круглосуточно каждые
пять минут, включая выходные и праздники.

Обе роли проверяются в
`scripts/tests/test_territory_timezones.py` (локальная разработка, файл не включён в этот выпуск);
правила самого импортёра — в [главе парсеров](04-сбор-данных-и-парсеры.md).

## История попыток импорта (22.09.2026)

Для сверенного восстановления старого отчёта `/import-result` принимает
`restore_attempts` и `expected_updated_at`. Восстановление допускается только
для завершённого отчёта без истории: дата и все счётчики последней попытки
должны совпасть с текущими. Повторное восстановление или изменившийся отчёт
возвращают 409. Время обновления и остаток очереди сохраняются.


Повторы VPS идут ежедневно каждый час с 12:00 до 20:00; первая попытка
остаётся за пятиминутным поллером. `IMPORT_SLOTS_LOCAL` всех территорий
должен совпадать с `court-import.timer`.

Исполнители дампов и точечных пачек передают в `/import-result` стабильный
`attempt_id` для одной попытки; VPS также передаёт `attempt_started_at`.
Повтор HTTP-отчёта использует тот же идентификатор. Завершённая успешная
попытка неизменяема: её повтор и поздний `started` не меняют итог и свежесть.
Запоздалый результат другой, более ранней попытки дополняет историю, но
не возвращает старое состояние очереди.

В записи журнала `attempts` хранит время, статус, `counts` и построчный отчёт
каждой попытки. Верхние числовые поля остаются результатом последней попытки:
`fetch_fail`, `card_failed` и `fetch_error` по-прежнему управляют очередью.
`totals` суммирует только подтверждённые действия: добавления обоих треков,
архивные добавления, связи, промоушены, возвраты и дочитки. Отсев, «уже в базе»
и ошибки повторов не суммируются. Пульт разделяет общий итог и последний
проход; при недочитанных карточках и история, и очередь показывают
«частично · ожидает повтора».

При первом обновлении старой записи её прежний результат сохраняется как
известный минимум с `attempt_history_incomplete=true`. Результаты более ранних
попыток нельзя восстановить из последнего отчёта: нужны логи и коммиты.
Проверенное восстановление помечается `attempt_history_recovered`.

VPS сохраняет неотправленные финальные отчёты в
`ops/mac-local-run/.runtime/import_results/` и повторяет отправку перед
чтением очереди. Если данные ещё не опубликованы, квитанция остаётся
`publication_pending=true`; после успешного push подтверждается та же
попытка. Пустой индекс Git не отменяет публикацию накопленных коммитов.
У аварийного GitHub-пути идентификатор включает `run_id` и `run_attempt`;
неопубликованные результаты в общий итог не входят.
...
Рег.облако / REG.RU (Free Tier до ~27.11.2026, дальше ~513 ₽/мес — решить; IP
...
пускает IP REG.RU (нормальная страница поиска). Шлюз пока ПЕРВЫЙ фолбэк
