...
резервом. Целевой сервер установки — Рег.облако (REG.RU), `195.19.66.234`, Ubuntu 24.04,
...
списка парсеров; остальные стартуют в порядке списка с интервалом 600 секунд.
При списке Урал → ХМАО → Башкортостан это 0/10/20 минут. Импорты в `import_all.sh`
